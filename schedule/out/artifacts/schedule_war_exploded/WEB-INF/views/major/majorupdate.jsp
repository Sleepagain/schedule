...
<body>
    ...
    <div class="alert alert-info">当前位置<b class="tip"></b>专业管理<b class="tip"></b>修改</div>
<form action="<%=basePath %>major/majorupdate.html" method="post">
    <table class="table table-striped table-bordered table-condensed list">
        ...
        <tbody>
        	<tr>
                ...
                <td>
                    <select name="pid" id="pid" >
                    	<c:forEach var="major1" items="${list}" varStatus="status">
                    		<option value="${major1.id}" <c:if test="${major1.id == major.pid}"> selected="selected"</c:if>>${major1.name}</option>
                    	</c:forEach>
                    </select>
                </td>
            </tr>
            ...
            <tr>
                <td >专业课程<font color="FF0000">*</font></td>
                <td>
                	<c:forEach var="classes" items="${classeslist}" varStatus="status">
                		
                    	<label><input name="kc" type="checkbox" value="${classes.type}" <c:if test="${fn:contains(major.classes,classes.type)}">checked="checked"</c:if>/>${classes.type} </label> 
                    </c:forEach>
                </td>
            </tr>
            <tr>
                <td >专业简介<font color="FF0000">*</font></td>
                <td>
